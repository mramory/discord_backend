...
export class ConversationService {
  ...
  async getConversation(conversationId = -1, userId = -1, currentUserId = -1) {
  ...
  async createConversation(req: Request, userId: number) {
    const currentUser = this.authService.checkIsAuth(req) as JwtPayload;
    const existConversation = await this.getConversation(
      null,
      userId,
      currentUser.id,
    );
    ...
    this.pusherService.trigger(
      `user__${userId}__newConversation`,
      'newConversation',
      newConversation,
    );
  ...
  async deleteConversation(id: number) {
    ...
    users.forEach((user) => {
      this.pusherService.trigger(
        `user__${user.id}__deleteConversation`,
        'deleteConversation',
        deleted,
      );
    });
